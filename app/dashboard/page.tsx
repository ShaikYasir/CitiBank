...
import { CreditCard, DollarSign, Home, PiggyBank, TrendingUp } from 'lucide-react'
    ...
    <div className="flex flex-col min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
          {[
            { title: 'Total Balance', value: '₹4,50,231.89', icon: DollarSign, color: 'bg-green-500' },
            { title: 'Investments', value: '₹40,345.67', icon: PiggyBank, color: 'bg-blue-500' },
            { title: 'Credit Card', value: '₹3,456.78', icon: CreditCard, color: 'bg-yellow-500' },
            { title: 'Loan', value: '₹2,34,567.89', icon: Home, color: 'bg-purple-500' },
          ].map((item, index) => (
            <motion.div
              key={item.title}
              initial={{ opacity: 0, y: 20 }}
